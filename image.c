...
drawImage()
{
    ...
    for (j = 0; j < n_terminal_image; j++)
    {
        ...
        if (enable_inline_image)
        {
            ...
            int w = i->cache->a_width > 0 ? ((i->cache->width +
                                              i->x % pixel_per_char_i +
                                              pixel_per_char_i -
                                              1) / pixel_per_char_i) : 0;
            int h =
                i->cache->a_height >
                0
                ? ((i->cache->height + i->y % pixel_per_line_i +
                    pixel_per_line_i - 1) / pixel_per_line_i) : 0;

            int sx = i->sx / pixel_per_char_i;
            int sy = i->sy / pixel_per_line_i;

            int sw =
                (i->width + i->sx % pixel_per_char_i + pixel_per_char_i -
                 1) / pixel_per_char_i;
            int sh =
                (i->height + i->sy % pixel_per_line_i + pixel_per_line_i -
                 1) / pixel_per_line_i;

#if 0
            fprintf(stderr,
                    "file %s x %d y %d w %d h %d sx %d sy %d sw %d sh %d (ppc %d ppl %d)\n",
                    i->cache->file, x, y, w, h, sx, sy, sw, sh,
            ...
            {
                put_image_sixel(url, x, y, w, h, sx, sy, sw, sh,
                                n_terminal_image);
            }
            ...
            {
                put_image_kitty(url, x, y, i->width, i->height, i->sx, i->sy,
                                sw * pixel_per_char, sh * pixel_per_line_i, sw,
                                sh);
            }
    ...
    if (!cache)
    {
        ...
        if (enable_inline_image == INLINE_IMG_OSC5379)
        {
            if (image->width > 0 && image->width % pixel_per_char_i > 0)
                image->width +=
                    (pixel_per_char_i - image->width % pixel_per_char_i);

            if (image->height > 0 && image->height % pixel_per_line_i > 0)
                image->height +=
                    (pixel_per_line_i - image->height % pixel_per_line_i);
...
parseImageHeader(char *path, u_int * width, u_int * height)
{
    ...
    if (memcmp(buf, "\xff\xd8", 2) == 0)
    {
        ...
        while (fread(buf, 1, 2, fp) == 2)
        {
            ...
            if (fread(buf, 1, 2, fp) == 2 &&
                /* SOF0 or SOF2 */
                (memcmp(buf, "\xff\xc0", 2) == 0
                 || memcmp(buf, "\xff\xc2", 2) == 0))
            {
                fseek(fp, 3, SEEK_CUR);
                if (fread(buf, 1, 2, fp) == 2)
                {
  ...
  got_image_size:
    ...
    else if (cache->width < 0)
    {
        ...
        cache->a_width = cache->width =
            (tmp > MAX_IMAGE_SIZE) ? MAX_IMAGE_SIZE : tmp;
    }
