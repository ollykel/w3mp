               ...
               HRequest * hr, FormList * request)
{
    ...
    if (qop_i >= QOP_AUTH)
    {
        Strcat_m_charp(tmp, ", qop=",
                       qop_i == QOP_AUTH ? "auth" : "auth-int", NULL);
        /* XXX how to count? */
...
getAuthCookie(struct http_auth *hauth, char *auth_header,
              ...
              FormList * request, volatile Str * uname, volatile Str * pwd)
{
    Str ss = NULL;
    Str tmp;
    TextListItem *i;
  ...
  load_doc:
         ...
         && !check_no_proxy(pu.host)))
    {
        ...
        if (fmInitialized)
        {
            term_cbreak();
            /* FIXME: gettextize? */
            message(Sprintf("%s contacted. Waiting for reply...", pu.host)->ptr,
                    0, 0);
            refresh();
        }
        if (t_buf == NULL)
            ...
            http_response_code == 407)
        {
                ...
                && (realm = get_auth_param(hauth.param, "realm")) != NULL)
            {
                ...
                add_auth_cookie_flag = 1;
                status = HTST_NORMAL;
                add_auth_user_passwd(auth_pu, qstr_unquote(realm)->ptr, uname,
                                     pwd, 1);
                goto load_doc;
            }
        }
...
#define APPEND(str) \
    ...
    if (!hidden_input && obuf->input_alt.in)
    {
        ...
        tmp =
            Sprintf
            ("<INPUT_ALT hseq=\"%d\" fid=\"%d\" name=\"%s\" type=\"%s\" value=\"%s\">",
    ...
    case HTML_LI:
        ...
        if (h_env->envc > 0)
        {
            ...
            case HTML_OL:
                if (parsedtag_get_value(tag, ATTR_TYPE, &p))
                    envs[h_env->envc].type = (int) *p;
                switch ((envs[h_env->envc].count >
                         0) ? envs[h_env->envc].type : '1')
                {
                case 'i':
...
#define MAX_VSPACE 1000
    ...
    case HTML_META:
        ...
        else if (p && q && !strcasecmp(p, "Content-Type") &&
                ...
                while (*p)
                {
                    ...
                    else
                    {
                        ...
                        if (--len)
                        {
                            ...
                            {
                                PSIZE;
                                PPUSH(mode | effect | ex_efct(ex_effect),
                                      *(p++));
                            }
                        }
#endif
...
#endif
static void
addnewline2(Buffer * buf, char *line, Lineprop * prop, Linecolor * color,
            int pos, int nlines)
{
    Line *l;
    l = New(Line);
...
static void
addnewline(Buffer * buf, char *line, Lineprop * prop, Linecolor * color,
...
loadHTMLstream(URLFile * f, Buffer * newBuf, FILE * src, int internal)
{
    ...
    else if (f->guess_type
             && !strcasecmp(f->guess_type, "application/xhtml+xml"))
        ...
        if (lineBuf2->length == 0)
        {
            ...
            if (buf->filename)
                buf->buffername = Sprintf("%s %s",
                                          CPIPEBUFFERNAME,
                                          conv_from_system(buf->filename))->ptr;
...
remove_dir(const char *dirname, size_t min_len, int should_zero_files)
{
    ...
    {
        fprintf(stderr,
                "ERROR: directory name too short (%s) should be at least %zu long\n\r",
                dirname, min_len);
        return -1;
    }
    ...
    while (file = readdir(dir))
    {
        ...
        if (file->d_type == DT_DIR
            && remove_dir(fname_full, min_len, should_zero_files) == -1)
            return -1;
        else
        {
