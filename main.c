...
DEFUN(pipeBufHtml, PIPE_BUF_HTML,
      "Pipe current buffer through a shell command and display output as html document")
{
    Buffer *buf;
    char *cmd, *tmpf;
    FILE *f;
...
DEFUN(pipesh, PIPE_SHELL,
      "Pipe current buffer to shell command and display output in terminal.")
{
    char *cmd;
    FILE *input;
...
DEFUN(readShHtml, READ_SHELL_HTML,
      "Execute shell command and display output as html document.")
{
    Buffer *buf;
    MySignalHandler(*prevtrap) ();
    char *cmd;
        ...
        else if (fi->parent->method == FORM_METHOD_POST)
        {
            ...
            {                   /* buf must be Currentbuf */
                ...
                buf->form_submit = save_submit_formlist(fi);
                 ...
                 || Currentbuf->bufferprop & BP_INTERNAL)
        {                       /* internal */
    ...
    case FORM_INPUT_RESET:
        for (i = 0; i < Currentbuf->formitem->nanchor; i++)
        {
            ...
            if (f2->parent == fi->parent &&
                ...
                f2->type != FORM_INPUT_HIDDEN && f2->type != FORM_INPUT_RESET)
            {
...
DEFUN(backBfSl, BACK_SILENT,
      "Close current buffer and return to the one below in stack, without error message")
{
    Buffer *buf = Currentbuf->linkBuffer[LB_N_FRAME];
                    ...
                    if (Currentbuf->cursorY == press_y - Currentbuf->rootY &&
                        (Currentbuf->cursorX == press_x - Currentbuf->rootX
                         ...
                         || (WcOption.use_wide &&
                             ...
                             (CharType
                              (Currentbuf->
                               currentLine->propBuf[Currentbuf->pos]) ==
                              PC_KANJI1)
                             && Currentbuf->cursorX ==
                             press_x - Currentbuf->rootX - 1)
#endif
                        ))
                    {
                        displayBuffer(Currentbuf, B_NORMAL);
...
DEFUN(promptResponse, PROMPT,
      "Prompt user for a string to send as part of a url in a GET request")
{
    _promptResponse(0);
}

// format: PROMPT_TAB url_fmt ["prompt"] ["default_response"]
DEFUN(promptResponseTab, PROMPT_TAB,
      "Prompt user for a string to send as part of a url in a GET request, and open in new tab")
{
    _promptResponse(1);
}
...
DEFUN(sourceSh, SOURCE_SHELL,
      "Execute shell command and interpret output as a series of commands")
{
    ...
    {
        source_read_amt =
            fread(source_data + source_read_count, 1, source_read_increment,
                  source_file);
        source_read_count += source_read_amt;
        source_data =
            realloc(source_data, source_read_count + source_read_increment + 1);
    }
...
DEFUN(reinit, REINIT, "Reload configuration file")
{
    ...
    disp_err_message(Sprintf
                     ("Don't know how to reinitialize '%s'", resource)->ptr,
                     FALSE);
}
    ...
    for (d = LastDL; d != NULL; d = d->prev)
    {
        ...
        if (!d->running)
        {
            ...
            case 0:
                if (size < d->size)
                    Strcat_charp(src,
                                 " Download ended but probably not complete");
                else
            ...
            case 2:
                Strcat_charp(src,
                             " Error: could not write to file (disk full)");
                break;
            default:
            ...
            else
            {
                if (post_file && i == 0)
                {
                    ...
                    if (fp != stdin)
                        fclose(fp);
                    request =
                        newFormList(NULL, "post", NULL, NULL, NULL, NULL, NULL);
