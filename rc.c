...
#endif
    {"simple_preserve_space", P_CHARINT, PI_ONOFF,
     (void *) &SimplePreserveSpace,
...
show_params(FILE * fp)
{
    ...
    for (j = 0; sections[j].name != NULL; j++)
    {
        ...
        if (!OptionEncode)
            cmt =
                wc_conv(_(sections[j].name), OptionCharset, InnerCharset)->ptr;
        else
#endif
            cmt = sections[j].name;
        ...
        while (sections[j].params[i].name)
        {
            ...
            fprintf(fp, "    -o %s=<%s>%*s%s\n",
                    sections[j].params[i].name, t, l, " ", conv_to_system(cmt));
            i++;
        }
    }
}
...
init_rc(void)
{
    ...
    if (!config_dir)
    {
        ...
        if (xdg_config_home && *xdg_config_home)
        {
            ...
            config_dir =
                Strnew_m_charp(xdg_config_home, "/", "w3mp", NULL)->ptr;
        }
        else
        {
...
load_option_panel(void)
{
    ...
    if (!OptionEncode)
    {
        ...
        {
            sections[i].name =
                wc_conv(_(sections[i].name), OptionCharset, InnerCharset)->ptr;
            for (p = sections[i].params; p->name; p++)
            {
                p->comment =
                    wc_conv(_(p->comment), OptionCharset, InnerCharset)->ptr;
